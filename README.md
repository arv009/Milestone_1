...
What challenged me on this code is how am I going to add an input as when I tried it a few times, it had an error and upon researching the problem, it leads me on using the while loop and add an input. At first, I thought it was wrong as it keeps loading but I noticed that on the box above it was asking me to input the temperature. However, when I tried to input a string on the input, the result is ValueError so I added a try and except. Upon adding the except, it solved the problem and it is asking to input a valid number.
